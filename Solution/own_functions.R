...
gradient_descent_2layer<- function(X,Y,Theta1,Theta2,Max_iter,X_test,y_test,lambda) {
  m = nrow(X)
  accuracy = matrix(0,1,1)
  ...
  for ( j in 1:Max_iter){
    ...
    for( i in 1:m1){
      ...
      temp_val_1 = Theta1_gradient
      temp_val_2 = Theta2_gradient
      temp_val_1[,1] = 0
      temp_val_2[,1] = 0
      
      
      Theta1 <- Theta1 - ((1/m)*((Theta1_gradient ) + (lambda * temp_val_1)))
      Theta2 <- Theta2 - ((1/m)*((Theta2_gradient ) + (lambda * temp_val_2)))
